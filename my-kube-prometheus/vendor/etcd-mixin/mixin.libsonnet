        ...
        rules: [
          {
            ...
            expr: |||
              count(up{%(etcd_selector)s} == 0) by (job,instance) > (count(up{%(etcd_selector)s}) by (job,instance) / 2 - 1)
            ||| % $._config,
            'for': '3m',
            labels: {
              severity: 'critical',
            ...
            annotations: {
              message: 'Etcd cluster "{{ $labels.job }}": insufficient members ({{ $value }}). Please pay attention on this instance:"{{ $labels.instance }}"',
            },
          },
          {
            alert: 'EtcdNoLeader',
          ...
          },
          {
            ...
            expr: |||
              100 * sum(rate(grpc_server_handled_total{%(etcd_selector)s, grpc_code!="OK", grpc_type="unary"}[5m])) BY (job, instance, grpc_service, grpc_method)
                /
              sum(rate(grpc_server_handled_total{grpc_type="unary", %(etcd_selector)s}[5m])) BY (job, instance, grpc_service, grpc_method)
                > 1
            ||| % $._config,
            'for': '10m',
            labels: {
